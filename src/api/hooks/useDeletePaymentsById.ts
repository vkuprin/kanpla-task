...
export const deletePaymentsByIdMutationKey = () => [{ url: '/payments/{id}' }] as const

export type DeletePaymentsByIdMutationKey = ReturnType<typeof deletePaymentsByIdMutationKey>

/**
 * {@link /payments/:id}
 */
async function deletePaymentsById(id: DeletePaymentsByIdPathParams['id'], headers: DeletePaymentsByIdHeaderParams, config: Partial<RequestConfig> = {}) {
...
export function useDeletePaymentsById(
  options: {
    mutation?: UseMutationOptions<
      ...
      { id: DeletePaymentsByIdPathParams['id']; headers: DeletePaymentsByIdHeaderParams }
    >
    client?: Partial<RequestConfig>
  } = {},
) {
  const { mutation: mutationOptions, client: config = {} } = options ?? {}
  const mutationKey = mutationOptions?.mutationKey ?? deletePaymentsByIdMutationKey()
