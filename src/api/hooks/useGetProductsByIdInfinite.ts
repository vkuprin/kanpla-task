...
export const getProductsByIdInfiniteQueryKey = (id: GetProductsByIdPathParams['id']) => [{ url: '/products/:id', params: { id: id } }] as const

export type GetProductsByIdInfiniteQueryKey = ReturnType<typeof getProductsByIdInfiniteQueryKey>
...
>(
  ...
  options: {
    query?: Partial<
      InfiniteQueryObserverOptions<ResponseConfig<GetProductsByIdQueryResponse>, ResponseErrorConfig<GetProductsById404>, TData, TQueryData, TQueryKey>
    >
    client?: Partial<RequestConfig>
  } = {},
) {
  const { query: queryOptions, client: config = {} } = options ?? {}
  const queryKey = queryOptions?.queryKey ?? getProductsByIdInfiniteQueryKey(id)

  const query = useInfiniteQuery({
    ...(getProductsByIdInfiniteQueryOptions(id, headers, config) as unknown as InfiniteQueryObserverOptions),
    queryKey,
    ...(queryOptions as unknown as Omit<InfiniteQueryObserverOptions, 'queryKey'>),
  }) as UseInfiniteQueryResult<TData, ResponseErrorConfig<GetProductsById404>> & { queryKey: TQueryKey }
