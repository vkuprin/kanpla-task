...
export function getProductsQueryOptions(headers: GetProductsHeaderParams, config: Partial<RequestConfig> = {}) {
...
>(
  ...
  options: {
    query?: Partial<QueryObserverOptions<ResponseConfig<GetProductsQueryResponse>, ResponseErrorConfig<GetProducts503>, TData, TQueryData, TQueryKey>>
  ...
  const query = useQuery({
    ...(getProductsQueryOptions(headers, config) as unknown as QueryObserverOptions),
    queryKey,
    ...(queryOptions as unknown as Omit<QueryObserverOptions, 'queryKey'>),
  }) as UseQueryResult<TData, ResponseErrorConfig<GetProducts503>> & { queryKey: TQueryKey }
